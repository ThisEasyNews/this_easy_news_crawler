...
from app.services.summarize import process_news_summaries
...
async def run_summary_process(limit: int):
    ...
    # BackgroundTask는 요청이 끝난 후 실행되므로 독립적인 DB 세션이 필요
    db = SessionLocal()
...
async def trigger_collection(background_tasks: BackgroundTasks):
    """모든 카테고리의 RSS 피드로부터 뉴스를 비동기로 수집"""
    try:
...
async def trigger_summarization(background_tasks: BackgroundTasks, limit: int = 5):
    """DB에 저장된 미요약 뉴스 중 'limit' 건만큼 GPT 요약"""
    try:
...
@router.post("/today-briefing", 
    ...
    responses={
        ...
        500: {"description": "서버 내부 오류 (GPT API 실패 또는 DB 제약 조건 위반)"} 
    }
    )
async def run_today_briefing(db: db_dep):
    """ 오늘의 뉴스 키워드를 분석하여 3분 브리핑을 생성"""
    try:
